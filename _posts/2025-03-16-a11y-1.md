...
사용자와 상호작용할 수 있는 요소를 정의하며, 이러한 역할을 JS를 사용해 구현해야 한다.다만 1️⃣과 마찬가지로, HTML 태그가 이 역할을 대신할 수 있다면, HTML 태그를 사용하는 것이 권장된다.
